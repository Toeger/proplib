...
void prop::Vertical_layout::set_name(std::string_view name) {
	name_updater = {[name = std::string{name.data(), name.size()}](decltype(children) &children) {
						std::size_t counter = 0;
						for (auto &child :
							 const_cast<std::remove_cvref_t<decltype(children.get())> &>(children.get())) {
							child->set_name(name + ".children[" + std::to_string(counter++) + "]");
						}
					},
					children};
